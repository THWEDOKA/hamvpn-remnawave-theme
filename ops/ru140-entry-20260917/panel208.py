"""Guarded existing-node migration: legacy USA plus four independently routed exits."""
import argparse
import copy
import hashlib
import json
...
from entry208_common import *

TIMER = 'ham-entry208-panel-rollback'
HOST_FIELDS = prior.HOST_FIELDS
LEGACY_EGRESS = '162.141.185.216'
...
def candidate(before, service_user):
    ...
    for n, inbound in zip(NODES, additional['inbounds']):
        ...
        config['outbounds'].append(prior.upstream(n, before['profiles'], service_user, False))
...
def snapshot(api):
    ...
    before = {'nodes': nodes, 'hosts': hosts, 'squads': api('GET', '/api/internal-squads/')['internalSquads'],
              'profiles': {p: api('GET', '/api/config-profiles/' + p) for p in {OLD_PROFILE, *[n['profile'] for n in NODES]}}}
    save('before', before)
    ...
    return {'snapshot_verified': True, 'legacy_preserved': True, 'four_separate_direct_exit_channels': True}
...
def clients(api, old=False):
    ...
    for n in NODES:
        if old:
            ...
            inbound = next(i for i in read('candidate')['inbounds'] if i['tag'] == 'vless-entry208-' + n['id'])
    ...
    for fp, sni in [('chrome', 'us3.torcalc.ru'), ('firefox', 'google.com')]:
...
def additions(squad, created):
    ...
    if squad['uuid'] != prior.read('backend-squad')['uuid']:
        result.extend(created['inbounds'][n['id']] for n in NODES if n['inbound'] in ids(squad))
    return result
...
def stage(api):
    ...
    created = {'profile': profile['uuid'],
               ...
               'inbounds': {n['id']: next(i['uuid'] for i in profile['inbounds'] if i['tag'] == 'vless-entry208-' + n['id']) for n in NODES}}
...
def activate(api):
    ...
    subprocess.run(['systemd-run', '--unit=' + TIMER, '--on-active=31m', 'python3', str(ROOT / 'panel208.py'), 'rollback'], check=True, capture_output=True)
...
def rollback(api):
    # On explicit rollback, stop node208's nginx router first. The paired node
...
def main():
    ...
    if args.action == 'export-backends':
        backend = api('GET', '/api/users/' + prior.read('backend-user')['uuid'])
        print(json.dumps([{'id': n['id'], 'ip': n['ip'], 'outbound': prior.upstream(n, read('before')['profiles'], backend, False)} for n in NODES])); return
