...
CONF = Path('/etc/ham-rs633-pl2')
DROP = Path('/etc/ssh/sshd_config.d/71-ham-rs633-pl2.conf')
UNIT = Path('/etc/systemd/system/ham-rs633-pl2.service')
HOME = Path('/var/lib/ham-rs633-pl2')
...
def generate():
    ...
    run('runuser', '-u', m.ACCOUNT, '--', 'ssh-keygen', '-q', '-t', 'ed25519', '-N', '',
        '-C', 'HAM-AEZA633-PL2-REVERSE', '-f', str(directory / 'id_ed25519'))
...
def identity():
    ...
    import re, base64
    require(re.fullmatch(r'ssh-ed25519 [A-Za-z0-9+/]+={0,2} HAM-AEZA633-PL2-REVERSE', public), 'Wrong public key')
...
def install():
    ...
    expanded = run('runuser', '-u', m.ACCOUNT, '--', 'ssh', '-G', '-F', str(CONF / 'ssh_config'), 'aeza-pl2').decode()
...
def retire_old():
    ...
    return {'old_pl2_unit_disabled': True, 'old_files_retained_for_recovery': True}
