...
def directory(revision):
    ...
    return '/opt/ham-reverse-nl5/releases/' + revision[:12] + '/' + t.SCOPE
...
def frontend(baseline=False):
    ...
    ips = {'main': m.EXIT, 'auto-member': m.EXIT, 'neighbor-18443': '217.60.68.182',
           'neighbor-18444': '31.56.188.150', 'neighbor-18446': '62.60.226.94'}
    def probe(item):
        label, wire = item
...
def ssh():
    ...
    pinned = (m.ENTRY + ' ' + ' '.join(public[:2])).encode()
