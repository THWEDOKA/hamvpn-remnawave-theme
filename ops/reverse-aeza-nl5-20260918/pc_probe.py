...
req=urllib.request.Request(s['subscriptionUrl'],headers={'User-Agent':'mihomo/1.19.29','X-Hwid':'ham-rs633-nl5-owned-probe','X-Device-Os':'iOS','X-Device-Model':'Deployment probe','X-Ver-Os':'18'})
...
'''.replace('DIR', repr(op.DIR))
    ...
    matches = [p for p in exported['proxies'] if p.get('server') == '193.233.222.244' and p.get('port') == 18445]
    assert matches and all(p.get('client-fingerprint') == 'firefox' for p in matches)
    proxy = next(p for p in matches if 'Нидерланды 5' in p['name'])
    proofs = []
    with tempfile.TemporaryDirectory(prefix='ham-nl5-mihomo-') as folder:
        ...
        try:
            ...
            for label, url in jobs:
                ...
                passed = q.returncode == 0 and (status == b'204' if label.startswith('204') else
                    body.strip() == b'94.183.255.82' if label == 'egress' else status == b'200' and len(body) >= 3145728)
