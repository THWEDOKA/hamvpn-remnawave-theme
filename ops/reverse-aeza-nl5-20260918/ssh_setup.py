...
CONF = Path('/etc/ham-rs633-nl5')
DROP = Path('/etc/ssh/sshd_config.d/71-ham-rs633-nl5.conf')
UNIT = Path('/etc/systemd/system/ham-rs633-nl5.service')
HOME = Path('/var/lib/ham-rs633-nl5')
...
def snapshot():
    ...
    port = m.R if which == 'entry' else m.B
    require(not run('ss', '-H', '-lnt', 'sport = :' + str(port)).strip(), 'Proposed port already occupied')
...
def generate():
    ...
    run('runuser', '-u', m.ACCOUNT, '--', 'ssh-keygen', '-q', '-t', 'ed25519', '-N', '',
        '-C', 'HAM-AEZA633-NL5-REVERSE', '-f', str(directory / 'id_ed25519'))
...
def identity():
    ...
    import re, base64
    require(re.fullmatch(r'ssh-ed25519 [A-Za-z0-9+/]+={0,2} HAM-AEZA633-NL5-REVERSE', public), 'Wrong public key')
...
def install():
    ...
    expanded = run('runuser', '-u', m.ACCOUNT, '--', 'ssh', '-G', '-F', str(CONF / 'ssh_config'), 'aeza-nl5').decode()
...
def retire_old():
    ...
    return {'old_nl5_unit_disabled': True, 'old_files_retained_for_recovery': True}
