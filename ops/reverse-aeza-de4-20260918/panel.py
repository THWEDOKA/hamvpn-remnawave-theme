...
EXIT_NODE = '98e15d09-88af-4c8f-8a97-6c2a62d36312'
EP = '8deddcaa-fd80-46ed-ac48-e4c5e93b0271'
XP = '23829877-f156-4599-a4ce-a5bebf8d37cf'
HOST = '6f4f2a94-d4b2-40ff-800e-de93d725567c'
HIDDEN = 'a05e6791-1081-44b5-8627-772bf3c5617c'
NEIGHBORS = ['d2a759f8-e9b6-4b7c-9e91-c1663a2fb39d', 'fb8c9f85-07d9-4248-99c2-c1163c853bee', '4495a293-bf41-401e-a723-445d5b5a9c5e']
TIMER = 'ham-rs633-de4-rollback'
...
def prepare():
    ...
    require(all(hosts[h]['address'] == m.ENTRY and hosts[h]['port'] == 18446 for h in [HOST, HIDDEN]), 'Target host drift')
    ...
    save('old-wire', next(o for o in profiles[EP]['config']['outbounds'] if o.get('tag') == 'exit-de94'))
...
def create_probe():
    ...
    ident = str(uuid.uuid4()); body = {'uuid': ident, 'username': 'rs633_de4_probe_' + ident[:8],
        'expireAt': (datetime.now(timezone.utc) + timedelta(hours=3)).isoformat(),
        'trafficLimitBytes': 1073741824, 'trafficLimitStrategy': 'NO_RESET', 'tag': 'RS633_DE4_PROBE',
        'description': 'Temporary reverse DE4 validation', 'activeInternalSquads': [squads[0]]}
...
def exports():
    ...
    req = urllib.request.Request(sub['subscriptionUrl'], headers={'User-Agent': 'Happ/5.7.0',
        'X-Hwid': 'ham-rs633-de4-owned-probe', 'X-Device-Os': 'iOS', 'X-Device-Model': 'Deployment probe', 'X-Ver-Os': '18'})
    ...
    for c in configs:
        ...
        for o in c.get('outbounds', []):
            if any(v.get('address') == m.ENTRY and v.get('port') == 18446 for v in o.get('settings', {}).get('vnext', [])): auto.append(o)
...
def stage():
    ...
    s = api('POST', '/api/internal-squads/', {'name': 'HAM-RS633-DE4-SVC', 'inbounds': [inbound]})
    objects['squad'] = s['uuid']; save('objects', objects)
    body = {'uuid': plan['service_id'], 'username': 'ham_rs633_de4_service', 'status': 'ACTIVE',
        'expireAt': '2036-09-18T00:00:00Z', 'trafficLimitBytes': 0, 'trafficLimitStrategy': 'NO_RESET',
        'tag': 'RS633_DE4_SVC', 'description': 'Dedicated loopback reverse Reality service', 'activeInternalSquads': [s['uuid']]}
...
def apply():
    ...
    return {'only_de4_outbound_migrated': True}
