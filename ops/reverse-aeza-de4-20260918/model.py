...
EXIT = '62.60.226.94'
R, B = 34443, 30443
TAG = 'ham-de4-reverse-reality-backend'
ACCOUNT = 'ham-rs633-de4'
UNIT = 'ham-rs633-de4'
...
def backend(config, private, short):
    ...
    out['inbounds'].append({'tag': TAG, 'listen': '127.0.0.1', 'port': B,
        ...
        'streamSettings': {'network': 'raw', 'security': 'reality', 'realitySettings': {
            ...
            'serverNames': ['de94.torcalc.ru'], 'privateKey': private, 'shortIds': [short],
            'minClientVer': '1.8.2'}}})
    return out


def wire(identity, public, short):
    return {'tag': 'exit-de94', 'protocol': 'vless', 'settings': {'vnext': [{
        ...
        'network': 'raw', 'security': 'reality', 'realitySettings': {
            'serverName': 'de94.torcalc.ru', 'fingerprint': 'firefox',
            'publicKey': public, 'shortId': short}}}


def entry(config, outbound):
    out = deepcopy(config)
    indices = [i for i, o in enumerate(out['outbounds']) if o.get('tag') == 'exit-de94']
    assert len(indices) == 1 and outbound['tag'] == 'exit-de94'
...
def sshd():
    policy = {'AuthenticationMethods': 'publickey', 'PubkeyAuthentication': 'yes',
        'AuthorizedKeysFile': '/etc/ham-rs633-de4/authorized_keys',
...
def unit():
    return f'''[Unit]
Description=HAM Germany-4 dedicated native reverse SSH Reality channel
...
User={ACCOUNT}
Group={ACCOUNT}
ExecStart=/usr/bin/ssh -NT -F /etc/{UNIT}/ssh_config aeza-de4
Restart=always
RestartSec=3
TimeoutStopSec=10
NoNewPrivileges=yes
