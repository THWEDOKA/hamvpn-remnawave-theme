...
EXIT_NODE = '2c0f93bf-ec0a-4dff-b0f1-b57badd920e1'
EP = '8deddcaa-fd80-46ed-ac48-e4c5e93b0271'
XP = '597b8c2d-65a6-40a5-bb97-2fb633157812'
HOST = 'd2a759f8-e9b6-4b7c-9e91-c1663a2fb39d'
HIDDEN = 'c3992dff-826a-48de-98ac-72d7a3d89fb9'
NEIGHBORS = ["fb8c9f85-07d9-4248-99c2-c1163c853bee","4495a293-bf41-401e-a723-445d5b5a9c5e","6f4f2a94-d4b2-40ff-800e-de93d725567c"]
TIMER = 'ham-rs633-de3-rollback'
...
def prepare():
    ...
    require(all(hosts[h]['address'] == m.ENTRY and hosts[h]['port'] == 18443 for h in [HOST, HIDDEN]), 'Target host drift')
    ...
    save('old-wire', next(o for o in profiles[EP]['config']['outbounds'] if o.get('tag') == 'exit-de182'))
...
def create_probe():
    ...
    ident = str(uuid.uuid4()); body = {'uuid': ident, 'username': 'rs633_de3_probe_' + ident[:8],
        'expireAt': (datetime.now(timezone.utc) + timedelta(hours=3)).isoformat(),
        'trafficLimitBytes': 1073741824, 'trafficLimitStrategy': 'NO_RESET', 'tag': 'RS633_DE3_PROBE',
        'description': 'Temporary reverse DE3 validation', 'activeInternalSquads': [squads[0]]}
...
def exports():
    ...
    req = urllib.request.Request(sub['subscriptionUrl'], headers={'User-Agent': 'Happ/5.7.0',
        'X-Hwid': 'ham-rs633-de3-owned-probe', 'X-Device-Os': 'iOS', 'X-Device-Model': 'Deployment probe', 'X-Ver-Os': '18'})
    ...
    for c in configs:
        ...
        for o in c.get('outbounds', []):
            if any(v.get('address') == m.ENTRY and v.get('port') == 18443 for v in o.get('settings', {}).get('vnext', [])): auto.append(o)
...
def stage():
    ...
    s = api('POST', '/api/internal-squads/', {'name': 'HAM-RS633-DE3-SVC', 'inbounds': [inbound]})
    objects['squad'] = s['uuid']; save('objects', objects)
    body = {'uuid': plan['service_id'], 'username': 'ham_rs633_de3_service', 'status': 'ACTIVE',
        'expireAt': '2036-09-18T00:00:00Z', 'trafficLimitBytes': 0, 'trafficLimitStrategy': 'NO_RESET',
        'tag': 'RS633_DE3_SVC', 'description': 'Dedicated loopback reverse Reality service', 'activeInternalSquads': [s['uuid']]}
...
def apply():
    ...
    return {'only_de3_outbound_migrated': True}
