...
EXIT = '217.60.68.182'
CLIENT_PORT = 18443
MAIN_LABEL = "Германия - 3"
SSH_PORT = 22222
R, B = 36443, 32443
TAG = 'ham-de3-reverse-reality-backend'
ACCOUNT = 'ham-rs633-de3'
UNIT = 'ham-rs633-de3'
...
def backend(config, private, short):
    ...
    out['inbounds'].append({'tag': TAG, 'listen': '127.0.0.1', 'port': B,
        ...
        'streamSettings': {'network': 'raw', 'security': 'reality', 'realitySettings': {
            ...
            'serverNames': ['de182.torcalc.ru'], 'privateKey': private, 'shortIds': [short],
            'minClientVer': '1.8.2'}}})
    return out


def wire(identity, public, short):
    return {'tag': 'exit-de182', 'protocol': 'vless', 'settings': {'vnext': [{
        ...
        'network': 'raw', 'security': 'reality', 'realitySettings': {
            'serverName': 'de182.torcalc.ru', 'fingerprint': 'firefox',
            'publicKey': public, 'shortId': short}}}


def entry(config, outbound):
    out = deepcopy(config)
    indices = [i for i, o in enumerate(out['outbounds']) if o.get('tag') == 'exit-de182']
    assert len(indices) == 1 and outbound['tag'] == 'exit-de182'
...
def sshd():
    policy = {'AuthenticationMethods': 'publickey', 'PubkeyAuthentication': 'yes',
        'AuthorizedKeysFile': '/etc/ham-rs633-de3/authorized_keys',
...
def unit():
    return f'''[Unit]
Description=HAM Germany-3 dedicated native reverse SSH Reality channel
...
User={ACCOUNT}
Group={ACCOUNT}
ExecStart=/usr/bin/ssh -NT -F /etc/{UNIT}/ssh_config aeza-de3
Restart=always
RestartSec=3
TimeoutStopSec=10
NoNewPrivileges=yes
