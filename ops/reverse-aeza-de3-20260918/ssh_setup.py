...
CONF = Path('/etc/ham-rs633-de3')
DROP = Path('/etc/ssh/sshd_config.d/71-ham-rs633-de3.conf')
UNIT = Path('/etc/systemd/system/ham-rs633-de3.service')
HOME = Path('/var/lib/ham-rs633-de3')
...
def snapshot():
    ...
    require(not run('ss', '-H', '-lnt', 'sport = :' + str(m.R if which == 'entry' else m.B)).strip(), 'Proposed port occupied')
    files = [Path('/etc/ssh/sshd_config'), *Path('/etc/ssh/sshd_config.d').glob('*.conf'),
             Path('/etc/systemd/system/ham-entry244-reverse.service'),
             Path('/etc/ham-rs633-pair/sshd_config'),
             Path('/etc/systemd/system/ham-rs633-pair-sshd.service')]
...
def generate():
    ...
    run('runuser', '-u', m.ACCOUNT, '--', 'ssh-keygen', '-q', '-t', 'ed25519', '-N', '',
        '-C', 'HAM-AEZA633-DE3-REVERSE', '-f', str(directory / 'id_ed25519'))
...
def identity():
    ...
    import re, base64
    require(re.fullmatch(r'ssh-ed25519 [A-Za-z0-9+/]+={0,2} HAM-AEZA633-DE3-REVERSE', public), 'Wrong public key')
    ...
    separate = '/etc/ham-rs633-pair/sshd_config'
    try:
        run('/usr/sbin/sshd', '-t', '-f', separate)
        require(all(effective(u, s) == before[u + '@' + s] for u in users for s in sources), 'Primary SSH policy changed')
        raw = run('/usr/sbin/sshd', '-T', '-f', separate, '-C', f'user={m.ACCOUNT},addr={m.EXIT},host=aeza633').decode()
        current = dict(line.split(' ', 1) for line in raw.splitlines() if ' ' in line)
        require(all(current.get(k.lower()) == value for k, value in expected.items()), 'Separate effective policy mismatch')
    ...
    return {'entry_restricted_identity_ready': True, 'primary_ssh_unchanged': True}


def install():
    require(role() == 'exit', 'Exit only'); read('generated')
    pinned = sys.stdin.read().strip()
    require(pinned.startswith('[' + m.ENTRY + ']:' + str(m.SSH_PORT) + ' ssh-ed25519 '), 'Wrong entry pin')
    ...
    expanded = run('runuser', '-u', m.ACCOUNT, '--', 'ssh', '-G', '-F', str(CONF / 'ssh_config'), 'aeza-de3').decode()
...
def listener():
    ...
    for _ in range(35):
        ...
        if len(lines) == 1 and lines[0].split()[3] == '127.0.0.1:' + str(m.R):
            return {'exact_loopback_listener': True, 'port': m.R}
        require(not lines, 'Unexpected listener binding')
        time.sleep(1)
    raise RuntimeError('Reverse listener missing after bounded startup')
...
def retire_old():
    ...
    return {'old_de3_unit_disabled': True, 'old_files_retained_for_recovery': True}
